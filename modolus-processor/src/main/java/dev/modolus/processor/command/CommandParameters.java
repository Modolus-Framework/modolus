...
public final class CommandParameters {
  ...
  public static List<ParameterSpec> getOverrideMethodParameters(@NotNull Command command) {
    return switch (command) {
      ...
      case Command c when c.target() == Command.CommandTarget.WORLD ->
          getAbstractWorldCommandParameters();
  ...
  @Contract(" -> new")
  private static @NotNull @Unmodifiable List<ParameterSpec>
      getAbstractAsyncWorldCommandParameters() {
    return List.of(getCommandContextParameter(), getWorldParameter());
  }

  @Contract(" -> new")
  private static @NotNull @Unmodifiable List<ParameterSpec> getAbstractWorldCommandParameters() {
    return List.of(getCommandContextParameter(), getWorldParameter(), getEntityStoreParameter());
  }
  ...
  private static @NotNull ParameterSpec getEntityStoreParameter() {
    var type =
        ParameterizedTypeName.get(
            ClassName.get(COMPONENT_PACKAGE, STORE_TYPE_NAME), getEntityStoreType());
  ...
  private static @NotNull ParameterSpec getWorldParameter() {
    ...
    return ParameterSpec.builder(type, WORLD_PARAM_NAME).addAnnotation(NotNull.class).build();
  }

  private static @NotNull ParameterSpec getEntityStoreRefParameter() {
    var type =
        ParameterizedTypeName.get(
            ClassName.get(COMPONENT_PACKAGE, ENTITY_STORE_REF_TYPE_NAME), getEntityStoreType());
  ...
  private static @NotNull ParameterSpec getPlayerRefParameter() {
    ...
    return ParameterSpec.builder(type, PLAYER_REF_PARAM_NAME).addAnnotation(NotNull.class).build();
  }
