...
package dev.modolus.processor.event;
...
public final class EventProcessor extends Processor {
  ...
  private static final ClassName LAZY_CLASS_NAME =
      ClassName.get("dev.modolus.util.singleton", "Lazy");
  private static final ClassName EVENT_LISTENER_CLASS_NAME =
      ClassName.get("dev.modolus.core.event", "EventListener");

  private final TypeMirror objectType =
      processingEnv.getElementUtils().getTypeElement("java.lang.Object").asType();
  private final TypeMirror iBaseEventType =
      processingEnv
          .getElementUtils()
          .getTypeElement("com.hypixel.hytale.event.IBaseEvent")
          .asType();
  private final TypeMirror iAsyncEventType =
      processingEnv
          .getElementUtils()
          .getTypeElement("com.hypixel.hytale.event.IAsyncEvent")
          .asType();
  private final TypeMirror voidType =
      processingEnv.getElementUtils().getTypeElement("java.lang.Void").asType();

  private final MethodSpec.Builder initializationBuilder =
  ...
  public void processSingle(
    ...
    annotated.getEnclosedElements().stream()
  ...
  private boolean isEventHandler(@NotNull Element enclosed) {
    ...
    if (method.getParameters().size() != 1) {
      processingEnv
          .getMessager()
          .printMessage(
              Diagnostic.Kind.ERROR, "Event handler method must have exactly one parameter");
      return false;
    }
  ...
  private void addAbstractMethod(
      @NotNull ExecutableElement method, @NotNull SourceFileWriter writer) {
    var abstractMethod =
        MethodSpec.methodBuilder(method.getSimpleName().toString())
            .addModifiers(Modifier.PROTECTED, Modifier.ABSTRACT)
            .returns(TypeName.get(method.getReturnType()))
            .addParameters(method.getParameters().stream().map(this::toParameterSpec).toList())
            .build();
  ...
  private void addInitializationMethod(@NotNull ExecutableElement method) {
    ...
    if (isGlobal(method))
      initializationBuilder.addStatement(
          "plugin.getEventRegistry().register$LGlobal($T.class, this::$L)",
          async,
          paramType,
          method.getSimpleName());
    else
      initializationBuilder.addStatement(
          "plugin.getEventRegistry().register$L($T.class, this::$L)",
          async,
          paramType,
          method.getSimpleName());
  }
  ...
  private @NotNull ParameterSpec toParameterSpec(@NotNull VariableElement variableElement) {
    return ParameterSpec.builder(
            TypeName.get(variableElement.asType()), variableElement.getSimpleName().toString())
        .build();
  }
